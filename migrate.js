 ...
 *   Create a file in migrations/ with format: {timestamp}_{name}.js or .sql
 ...
 *   Plain SQL file — executed as a single query block.
 */
...
async function runFolderMigrations(client) {
  ...
  for (const file of files) {
    ...
    const name = isSql
      ? file.replace('.sql', '')
      : (require(path.join(migrationsDir, file)).name || file.replace('.js', ''));
